...
public class AnimationUltil {
    private static final int ANIMATION_DURUTION = 1000;
    private static boolean isAnimationStart;
    public  static void translateAnimation(final ImageView viewAnimation, final View startView, View enView,
        ...
        float endViewWidthCenter = enView.getWidth() *0.75f;
        float endViewHeightCenter = enView.getHeight() * 2f;

        final int[] startPos = new int[2];
        startView.getLocationOnScreen(startPos);
        final  int[] endPos = new int[2];
        enView.getLocationOnScreen(endPos);
